"""Native cognis-connect emit for cloudbill — forward findings to any platform.

Maps cloudbill's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    cloudbill ... --format json | cloudbill-emit --to stix
    cloudbill-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('resource', '') + ' ' + rec.get('service', '')
        out['severity'] = 'info'
        if rec.get('anomaly', False):
            out['severity'] = 'critical'
        elif rec.get('high', False):
            out['severity'] = 'high'
        elif rec.get('medium', False):
            out['severity'] = 'medium'
        elif rec.get('low', False):
            out['severity'] = 'low'
        out['type'] = 'cost'
        out['description'] = f"Cost anomaly detected in {rec.get('resource', '')}"
        out['tags'] = [rec.get('cloud'), rec.get('region')]
